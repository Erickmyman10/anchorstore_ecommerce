...
import { ShoppingCart, Menu, X, Search, User, LogOut, Package, Settings as SettingsIcon, LayoutDashboard } from 'lucide-react';
...
const Navbar = () => {
  ...
  // Derive display name: prefer profile full name, fall back to username then email prefix
            ...
            {isAuthenticated ? (
              <div className="relative" ref={accountRef}>
                ...
                >
                  ...
                  <span className="hidden sm:block text-sm font-bold text-gray-800">My Account</span>
                </button>

                {/* Dropdown */}
                {accountOpen && (
                  <div className="absolute right-0 top-full mt-2 w-52 bg-white rounded-xl border border-gray-100 shadow-card-hover py-1 z-50 animate-[chatSlideUp_0.15s_ease]">
              ...
              <Link
                to="/login"
                className="flex items-center gap-2 px-3 py-2 rounded-xl text-gray-500 hover:bg-gray-100 hover:text-gray-900 transition-all duration-200"
                aria-label="Sign in"
              >
                <User className="w-5 h-5" />
                <span className="hidden sm:block text-sm font-bold">My Account</span>
              </Link>
            )}
          </div>
