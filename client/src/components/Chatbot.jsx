  ...
  return (
    <div className="cb-product-card">
      ...
      <div className="cb-product-info">
        <p className="cb-product-name">{product.name || product.title}</p>
        <p className="cb-product-price">{fmt(product.unitPrice)}</p>
        <button
...
export default function Chatbot() {
  ...
  const handleAddToCart = useCallback((product) => {
    ...
    const name = (product.name || product.title || '').split(" ").slice(0, 4).join(" ");
