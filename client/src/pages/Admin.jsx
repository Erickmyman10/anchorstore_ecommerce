...
const Admin = () => {
    ...
    <div className="border-t border-brand-100 bg-brand-50/30 px-4 py-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 mb-4">
        {order.items.map((item, idx) => (
          <div key={idx} className="bg-white border border-gray-100 rounded-xl px-4 py-3 flex items-center justify-between shadow-sm">
            <div className="min-w-0 pr-3">
              <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide">Product ID</p>
              <p className="text-xs font-bold text-gray-700 font-mono truncate max-w-35">{item.productId}</p>
      ...
      {order.payment && (
        <div className="flex flex-wrap gap-4 text-xs text-gray-500 border-t border-brand-100 pt-3">
          <span>Payment: <span className="font-bold text-gray-700 capitalize">{order.payment.method.replace(/_/g, ' ')}</span></span>
          <span>Status: <span className={`font-bold capitalize ${
            ...
            : 'text-yellow-600'
          }`}>{order.payment.status}</span></span>
          <span>ID: <span className="font-mono text-gray-600">{order.payment.id}</span></span>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="max-w-7xl mx-auto px-0 sm:px-2 lg:px-4 py-4 sm:py-8">
      ...
      {/* ── Page header ── */}
      <div className="flex items-start justify-between mb-6 sm:mb-8 px-2 sm:px-0">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <ShieldCheck className="w-4 h-4 text-brand-500" />
            <span className="text-xs font-bold text-brand-500 uppercase tracking-widest">Admin Panel</span>
          </div>
          <h1 className="text-xl sm:text-2xl font-extrabold text-gray-900">Order Management</h1>
          <p className="text-sm text-gray-400 mt-0.5">{orders.length} orders · {fmt(revenue)} revenue</p>
        </div>
      </div>

      {/* ── Stats cards ── */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-3 mb-6 sm:mb-8 px-2 sm:px-0">
        {ALL_STATUSES.map((s) => {
          const cfg    = STATUS[s];
          const Icon   = cfg.Icon;
          ...
          return (
            <button
              ...
              className={`rounded-2xl border p-3 sm:p-4 text-left transition-all duration-150 ${
                active
            ...
            >
              ...
              <p className="text-xl sm:text-2xl font-extrabold text-gray-900">{counts[s]}</p>
      ...
      {/* ── Toolbar ── */}
      <div className="flex flex-col gap-3 mb-4 px-2 sm:px-0">
      ...
      {/* ── Orders content ── */}
      {filtered.length === 0 ? (
        <div className="text-center py-24 text-gray-400 px-2 sm:px-0">
        ...
        <>
          ...
          <div className="md:hidden bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden divide-y divide-gray-50 mx-2 sm:mx-0">
            {filtered.map((order) => (
              <div key={order.id}>
                <div
                  className={`p-4 cursor-pointer transition-colors ${expanded === order.id ? 'bg-brand-50/40' : 'hover:bg-gray-50/60'}`}
                  onClick={() => toggle(order.id)}
                >
                  {/* Row 1: tracking code + status + chevron */}
                  <div className="flex items-start justify-between mb-3">
                    <div className="min-w-0">
                      <p className="font-bold text-brand-600 text-sm">{order.trackingCode}</p>
                      <p className="font-mono text-xs text-gray-400 truncate max-w-40 mt-0.5">{order.id}</p>
                    </div>
                  ...
                  <div onClick={(e) => e.stopPropagation()}>
                    ...
                    <select
                      ...
                      className="w-full text-xs bg-white border border-gray-200 text-gray-700 rounded-lg px-3 py-2 focus:ring-2 focus:ring-brand-500 outline-none"
                    >
                      {ALL_STATUSES.map((s) => (
                ...
                {expanded === order.id && <ExpandedPanel order={order} />}
              </div>
            ))}
          </div>
                ...
                <tbody className="divide-y divide-gray-50">
                  {filtered.map((order) => (
                    <React.Fragment key={order.id}>
                      <tr
                        className={`hover:bg-gray-50/60 transition-colors cursor-pointer ${expanded === order.id ? 'bg-brand-50/40' : ''}`}
                        onClick={() => toggle(order.id)}
                      >
                        <td className="px-4 py-3.5">
                          <span className="font-mono text-xs text-gray-400 block max-w-24 truncate" title={order.id}>{order.id}</span>
                        </td>
                        <td className="px-4 py-3.5">
                          <span className="font-bold text-brand-600 text-xs">{order.trackingCode}</span>
                        </td>
                        <td className="px-4 py-3.5">
                          <span className="font-mono text-xs text-gray-400 block max-w-24 truncate" title={order.customerId}>{order.customerId}</span>
                        </td>
                        <td className="px-4 py-3.5">
