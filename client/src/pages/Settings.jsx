...
function StatusMsg({ status, successLabel = 'Saved' }) {
  if (!status) return null;
  if (status === 'saved') {
    return (
      <span className="flex items-center gap-1.5 text-sm text-green-600 font-semibold">
        <CheckCircle2 className="w-4 h-4 shrink-0" /> {successLabel}
      </span>
    );
  }
  return (
...
function SettingsCard({ title, status, onSave, btnLabel = 'Save', successLabel, children }) {
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-6">
      <h2 className="text-base font-bold text-gray-900 mb-5">{title}</h2>
      <div className="space-y-4">{children}</div>
      <div className="flex items-center justify-between mt-5 pt-4 border-t border-gray-100">
        <StatusMsg status={status} successLabel={successLabel} />
        <button
          onClick={onSave}
...
export default function Settings() {
  ...
  const saveProfile = async () => {
    if (!name.trim()) { flash(setProfileStatus, { error: 'Name cannot be empty.' }); return; }
    try {
      await updateProfile({ name: name.trim() });
      flash(setProfileStatus, 'saved');
    } catch (err) {
      flash(setProfileStatus, { error: err.message });
    }
  };

  const saveAddress = async () => {
    try {
      await updateProfile({ address: `${address.street}, ${address.city}, ${address.state}` });
      flash(setAddressStatus, 'saved');
    } catch (err) {
      flash(setAddressStatus, { error: err.message });
    }
  };

  const saveContact = async () => {
    const stripped = phone.replace(/\s/g, '');
    const isValid  = /^\+?[0-9]{7,15}$/.test(stripped);
    if (phone && !isValid) {
      flash(setContactStatus, { error: 'Enter a valid phone number.' });
      return;
    }
    try {
      await updateProfile({ phone: stripped });
      flash(setContactStatus, 'saved');
    } catch (err) {
      flash(setContactStatus, { error: err.message });
    }
  };
          ...
          >
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1">
                Nigerian (08012345678) or international (+447911123456)
              </p>
            </div>
          </SettingsCard>
